...
* `--blacklist-regex, -r` Regular expression to match services that are excluded from updates. Either `--label-enable` or `--blacklist-regex` could be defined, but not both. The list can also be provided by setting the `BLACKLIST` environment variable.
